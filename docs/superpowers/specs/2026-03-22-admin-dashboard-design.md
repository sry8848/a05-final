...
- 单个模块失败时不能拖垮整个首页
- 默认复用现有管理端视觉结构和版式，不以“重做后台 UI”为目标
- 前后端都应拆成边界清晰的小单元，避免继续把所有逻辑堆进现有超大组件

## 复用约束

首版 dashboard 的实现应尽量复用现有前端设计，不做大改版式。

明确约束如下：

- 继续复用 [App.vue](D:/a05-cursor/frontend/src/App.vue) 中现有的管理端入口切换逻辑
- 继续复用 [AdminLayout.vue](D:/a05-cursor/frontend/src/components/AdminLayout.vue) 的侧边栏、顶部栏、菜单结构和整体布局
- 继续复用 [AdminDashboard.vue](D:/a05-cursor/frontend/src/components/AdminDashboard.vue) 当前的“顶部卡片 + 中部趋势图 + 右侧信息区”页面骨架
- 继续复用当前管理端的视觉语言，包括 `glass-card`、现有图标体系、间距节奏和主色方案

本次实现允许的改动：

- 把写死的演示数据替换成真实接口数据
- 删除没有真实数据源支撑的演示模块
- 对现有模块标题、文案、指标名称做小范围修正
- 为了可维护性对组件做内部拆分，但拆分后页面视觉结构应基本保持不变
